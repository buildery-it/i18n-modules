...
const build = ({ keysRoot, moduleEnding, dictionaryPattern }) => {
  ...
  if (!fs.existsSync(dictionariesRoot)) fs.mkdirSync(dictionariesRoot, { recursive: true });

  const moduleRegex = new RegExp(`${escapeRegex(moduleEnding)}$`);
  const files = getFiles(keysRoot, moduleRegex);

  const generated = files.reduce((acc, filePath) => {
  ...
  debug('found %d module files, attempting to update dictionaries', files.length);
