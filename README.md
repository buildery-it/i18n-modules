...
CSS Modules for i18n. A webpack plugin that brings support for a modular approach to dictionaries. Especially useful for tools such as [Phraseapp](https://phrase.com). Pairs well with [react-polyglot](https://github.com/entva/react-polyglot).
...
```javascript
const I18nModules = require('@entva/i18n-modules');
//...
plugins: [
  new I18nModules(),
],
...
```javascript
const { loader: i18nLoader } = require('@entva/i18n-modules');
//...
module: {
  rules: [
    {
...
When using TypeScript with `resolveJsonModule` enabled, which is enforced by Next.js and many other frameworks, TypeScript will read JSON files from the disk "as is" without any knowledge of the modifications happening during build time. That makes typechecking these JSON files counterproductive and erroneously flags correct property lookups. There are 3 possible workarounds for the issue:

1. After importing language module pass it in a function like this, that overrides type definitions:

```typescript
  export const useNamespace = (dictionary: Record<string, unknown>) => (
    dictionary as typeof dictionary[keyof typeof dictionary]
  );
```

2. Instead of directly accessing object properties wrap your lookup in (lodash.get)[https://lodash.com/docs/#get] which will disable typechecking.

```typescript
  import get from 'lodash.get';

  const titleKey = get(lang, 'title');
```

3. Typecast language modules to `any` type.
...
## Options:
 ...
 - `emitFile` (optional) - determines if the bundled dictionary needs to also be stored on the disk. Set to `false` when you prefer to use `@entva/i18n-modules` binary to update original dictionaries or don't intend to store module build results anywhere. Defaults to `true`.
...
We support `I18N_MODULES_CONTEXT` env variable to set current working directory to something else. It works similarly to webpack's `context` option. When set both plugin and `@entva/i18n-modules` binary will look for the RC file relative to that context folder. In addition when running the `@entva/i18n-modules` binary all relative file paths in the RC file will be relative to that context. `I18N_MODULES_CONTEXT` itself accepts paths relative to current working directory, e.g. one could set `I18N_MODULES_CONTEXT=./frontend/app` and it would work.

### An example with Phraseapp CLI:

 - `npx @entva/i18n-modules build && phraseapp push` will push all generated modules to phraseapp
 - `phraseapp pull && npx @entva/i18n-modules update` will update all modules with changes from phraseapp
